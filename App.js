...
export default function App() {
  ...
  useEffect(() => {
    const loadSession = async () => {
      try {
        ...
        console.log('📦 loadSession userData:', userData ? 'found' : 'null');
        console.log('📦 loadSession roleData:', roleData);
  ...
  // ✅ FIXED: saves to storage first, then sets state to trigger re-render
  const handleLogin = async (userData, userRole) => {
    console.log('🔑 handleLogin called, role:', userRole, 'user:', userData?.email);
    try {
    ...
    // Set state AFTER storage to trigger re-render
    setUser(userData);
    setRole(userRole || 'client');
    console.log('✅ State updated, should navigate now');
  };
  ...
  // ─── No user or role: show login ──────────────────────────────────────────
  if (!user || !role) {
    console.log('🔒 Showing login screen. user:', !!user, 'role:', role);
    return (
      <View style={{ flex: 1 }}>
  ...
  console.log('🏠 Showing dashboard. role:', role, 'user:', user?.email);
...
const styles = StyleSheet.create({
  container: { flex: 1, backgroundColor: '#f8fafc', ...(Platform.OS === 'web' ? { height: '100vh' } : {}) },
  layout: { flex: 1, flexDirection: 'row', position: 'relative', overflow: 'hidden' },
  sidebar: { flexDirection: 'column', borderRightWidth: 1, overflow: 'hidden' },
  sidebarHeader: { flexDirection: 'row', alignItems: 'center', paddingHorizontal: 12, paddingVertical: 16, gap: 10, borderBottomWidth: 1, borderBottomColor: 'rgba(0,0,0,0.06)' },
  logoIconBox: { backgroundColor: '#0f172a', width: 34, height: 34, borderRadius: 8, justifyContent: 'center', alignItems: 'center', flexShrink: 0 },
  logoText: { color: '#ffffff', fontSize: 26, fontWeight: '800', letterSpacing: -0.4 },
  clientSubName: { fontSize: 15, color: '#ffffff', fontWeight: '600', marginTop: -1 },
  sectionHeading: { fontSize: 12, fontWeight: '700', color: '#ffffff', letterSpacing: 1.1, paddingHorizontal: 16, paddingTop: 12, paddingBottom: 4 },
  navItem: { flexDirection: 'row', alignItems: 'center', height: 42, marginHorizontal: 6, marginVertical: 1, paddingHorizontal: 10, borderRadius: 8, gap: 12, position: 'relative', overflow: 'hidden' },
  navItemActiveWhite: { backgroundColor: 'rgba(255,255,255,0.15)' },
  activePill: { position: 'absolute', left: -6, top: 10, bottom: 10, width: 3, borderRadius: 3 },
  navLabel: { fontSize: 16, fontWeight: '600', flex: 1 },
  sidebarFooter: { paddingBottom: 12, borderTopWidth: 1, borderTopColor: 'rgba(0,0,0,0.06)', paddingTop: 8 },
  collapseBtn: { flexDirection: 'row', alignItems: 'center', gap: 8, paddingHorizontal: 16, paddingVertical: 10 },
  collapseBtnText: { fontSize: 12, color: '#94a3b8', fontWeight: '500' },
  mainContent: { flex: 1, backgroundColor: '#f8fafc', overflow: 'hidden', flexDirection: 'column' },
  topBar: { height: 48, flexDirection: 'row', alignItems: 'center', paddingHorizontal: 12, backgroundColor: '#ffffff', borderBottomWidth: 1, borderBottomColor: '#e2e8f0' },
  hamburger: { width: 34, height: 34, borderRadius: 8, backgroundColor: '#f8fafc', borderWidth: 1, borderColor: '#e2e8f0', justifyContent: 'center', alignItems: 'center' },
  mobileOverlay: { ...StyleSheet.absoluteFillObject, backgroundColor: 'rgba(0,0,0,0.45)', zIndex: 99 },
  loadingContainer: { flex: 1, backgroundColor: '#f8fafc', justifyContent: 'center', alignItems: 'center' },
  roleSelection: { flex: 1 },
  adminSwitchBtn: { position: 'absolute', bottom: 40, alignSelf: 'center', flexDirection: 'row', alignItems: 'center', gap: 6, paddingVertical: 10, paddingHorizontal: 20 },
