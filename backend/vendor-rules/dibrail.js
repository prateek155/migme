...
const domConfig = {
  ...
  lineBasedFields: {
    orderNo: {
      match: /Order\s*(?:No|ID)\s*[:\-\*]*\s*#?(\d+)/i,
      transform: (v) => v.replace(/[^\d]/g, ""),
    },
    customerName: {
      match: /Customer\s*Name\s*[:\-\*]*\s*(.+)/i,
      transform: (v) => v.replace(/\*/g, "").trim(),
    },
    contactNo: {
      match: /Mobile\s*[:\-\*]*\s*(.+)/i,
      // Mobile field may have multiple comma-separated numbers —
      // always take the first valid 10-digit Indian mobile.
      transform: (v) => {
    ...
    coach: {
      match: /Coach\s*&?\s*Seat\s*[:\-\*]*\s*(.+)/i,
      // "S6  - 8" → collapse spaces, " - " → "/"
      transform: (v) =>
        v.replace(/\*/g, "").replace(/\s+/g, " ").trim().replace(/\s*-\s*/, "/"),
    },
    _deliveryRaw: {
      match: /Delivery\s*Time\s*[:\-\*]*\s*(\d{2}-\d{2}-\d{4}\s+\d{2}:\d{2})/i,
      transform: (v) => v.trim(),
    },
    orderTypeRaw: {
      match: /Order\s*Type\s*[:\-\*]*\s*(COD|ONLINE|PREPAID|PRE_PAID)/i,
      transform: (v) => v.trim().toUpperCase(),
    },
    totalAmount: {
      match: /Total\s*Amount\s*[:\-\*]*\s*([\d,]+\.?\d*)/i,
